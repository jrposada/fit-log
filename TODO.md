...
- [ ] **Hold points misplaced when zoomed** _(bug)_ — Tapping to place holds does not position them correctly if the image is zoomed in. Tap coordinates are not properly translated to image-space at non-1x zoom levels.
- [ ] **Edit mode renders at different scale than view mode** _(bug)_ — Edit mode does not display at full screen like view mode. Some holds fall outside the visible area. Needs analysis: fix scale or fix seeder data.
