...
Today the mobile app is organized **per sport**: a nested Climbing stack with home-grown sub-tabs (Logbook/Browse/Stats built on local `useState`, not real routes) and a bare Training screen. This doesn't scale — every new sport (starting with gym, more later) would mean cloning a whole nested layout, and the sub-tabs already diverge in quality between Climbing and Training.
...
| **Activity flows** | Sport-*specific*: log a climb, log a gym session | Behind the FAB and inside session detail |
...
Scope of this design set: the framework + the **climbing migration** are designed in full. Gym is captured as an entity sketch against a documented "add a sport" contract. Sports beyond climbing and gym are out of scope for now — not designed, not stubbed.
...
History   → merged feed of sessions, segmented filter: All · Climbing · Gym
...
| 6 | [new-sports](multi-sport-new-sports.md) | 1, 3 | The "add a sport" contract + the gym entity stub |
...
| Sport selection UX | In-context segmented filter, **not** a global header mode | Avoids hidden-mode errors; sports don't map uniformly onto tabs (gym has no map) |
...
| Sport breadth now | Climbing + Gym only; other sports deferred and undesigned | Locks the framework on two real sports before investing in flows for unbuilt ones |
