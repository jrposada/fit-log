...
2. **Stale-session UX** — deferred. The backend doesn't yet enforce staleness on the write path; when it does, add a resume/new variant here.
3. **Manual end** from the overlay popover — confirmation modal, then `useTrainingSessionPut({ id, endedAt: new Date() })`. No dedicated end endpoint — ending is just a regular update.
...
4. **Manual end** — overlay popover → End session → `PUT` with `endedAt` set; overlay returns to "Start session" state; detail page shows `endedAt`.
...
- **Active-session source** — derived via `useTrainingSessions({ active: true, limit: 1 })`. No dedicated endpoint or hook.
