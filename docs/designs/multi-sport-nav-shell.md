...
Part of the [multi-sport refactor](multi-sport-overview.md). Depends on the [activity-model](multi-sport-activity-model.md) API contract.
...
- **History** = a single screen listing `Activity` rows (via `useActivities`), newest first, rendered from each row's denormalized `summary`. A **segmented control** at the top filters by sport: `All · Climbing · Gym · Apnea · Padel`. Tapping a row opens the sport's session-detail screen.
...
- **Home** = the cross-sport dashboard: recent activity across sports + per-sport summary cards (the existing `home-screen.tsx` stat cards generalize to read `useActivitiesStats`). Ship a sensible **default** layout; customization is a later iteration, explicitly out of scope here.
...
| History unit | Activity (session) rows, uniform across sports | One list renderer; per-sport richness is in the detail screen |
