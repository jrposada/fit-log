...
Part of the [multi-sport refactor](multi-sport-overview.md). Depends on [activity-model](multi-sport-activity-model.md) (shared Location).
...
`Location` is now shared across sports. A location's relevant sport(s) are derived from the activities that reference it (a crag accrues climbing activities; a court accrues padel) — so the map can color/icon each pin by sport and filter without a manual per-location sport tag. If derivation proves expensive, denormalize a `sports: string[]` summary onto `Location`.
...
- Tapping a pin → location detail (existing `location-detail-screen.tsx`, generalized to show activities of any sport at that location, not just climbs).
...
| Pin sport | Derived from referencing activities (denormalize only if slow) | Avoids a manual per-location sport field that drifts from reality |
...
- Map is meaningless for gym/apnea — never let the map filter imply those sports exist there. Only show sports with geolocated activities.
