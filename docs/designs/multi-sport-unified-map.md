...
`Location` is now shared across sports, though today only climbing sessions reference one (gym has no location). A location's relevant sport(s) are derived from the sessions that reference it (a crag accrues climbing sessions) — so the map can color/icon each pin by sport and filter without a manual per-location sport tag. Deriving this across the per-sport collections is the same fan-out shape as the [feed](multi-sport-feed.md); if it proves expensive, denormalize a `sports: string[]` summary onto `Location` (maintained when sessions are written). This stays a general, sport-keyed design so a future location-bearing sport (e.g. an outdoor sport with courts/venues) slots in without rework.

Behavior:

- Pins for all the user's locations, icon/color by sport.
- A filter chip row (`All · Climbing · …`) — only sports that actually have geolocated activity appear (gym has none, so it never shows a chip).
...
| Placement | Reached from Home / Browse context, not its own tab | Keeps the bottom tab set at 4; map is sport-conditional and would be a dead tab for gym-only users |

## Gotchas

- Map is meaningless for gym — never let the map filter imply gym exists there. Only show sports with geolocated sessions.
